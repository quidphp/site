...
class Mailchimp extends Core\ServiceRequestAlias implements Site\Contract\Newsletter
{
	...
	public static $config = array(
		'option'=>array(
			...
			'responseCode'=>array(200,500), // le code de réponse peut être 200 ou 500
			'key'=>null, // apiKey pour mailchimp
			'list'=>null, // code de la liste
			'addLang'=>true, // si lang est ajouté au merge vars
			'subscribed'=>array('pending','subscribed'), // status considéré comme subscribed
			'mergeVars'=>array('firstName'=>'FNAME','lastName'=>'LNAME')), // remplacement pour les mergeVars
	...
	public function memberInfo(string $email,?array $post=null):?array
	{
		...
		if(Base\Validate::isEmail($email))
		{
			$post = (array) $post;
			$post['id'] = $list;
			$post['emails'] = array(array('email'=>$email));
	...
	public function members(?array $post=null):array
	{
		...
		while (true) 
		{
			$post['opts'] = array('start'=>$start,'limit'=>$limit);
	...
	public function subscribe(string $email,$vars=array(),?array $post=null,bool $isSubscribed=true):?array
	{
		$return = null;
		$list = $this->checkList();
		...
		if(Base\Validate::isEmail($email))
		{
			if($isSubscribed === false || !$this->isSubscribed($email))
			{
				$post = (array) $post;
				$post['id'] = $list;
				$post['email'] = array('email'=>$email);
				$post['merge_vars'] = array();
	...
	public function subscribeBool(string $email,$vars=array(),?array $post=null,bool $isSubscribed=true):bool 
	{
		$return = false;
