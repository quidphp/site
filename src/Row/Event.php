...
abstract class Event extends Core\RowAlias implements Main\Contract\Meta
{
	...
	public static $config = array(
		...
		'cols'=>array(
			'datetimeStart'=>array('required'=>true,'general'=>true),
			'datetimeEnd'=>array('required'=>true)),
		'@cms'=>array(
			'route'=>array(
	...
	{
		return Base\Date::formatStartEnd($this['datetimeStart'],$this['datetimeEnd'],Base\Arr::plus(array('format'=>1,'formatDay'=>0),$option));
	}
	...
	public static function specificOperation(self $row):string 
	{
		...
		if($row->table()->hasPermission('specificOperation'))
		{
			...
			{
				$segment = array('table'=>$table,'order'=>'id','direction'=>'desc','filter'=>array('event_id'=>$row));
