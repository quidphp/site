...
class Page extends Core\RowAlias implements Main\Contract\Meta
{
	...
	public static $config = array(
		'key'=>array('slug_[lang]','slugPath_[lang]',0),
		'cols'=>array(
			'slug_fr'=>array(
				'slug'=>array(self::class,'makeSlug'),
				'exists'=>false),
			'slug_en'=>array(
				'slug'=>array(self::class,'makeSlug'),
				'exists'=>false),
	...
	public function childs(bool $rows=true)
	{
		...
		if($this->hasParentCell())
		{
			...
			$return = $this->cache(__METHOD__,function() {
				...
				$table = $this->table();
				$where = $table->where(array($hierarchy=>$this));
				$order = $table->order();
	...
	public function tableRelationOutput():string
	{
		...
		if($this->inAllSegment())
		{
			...
			if(!empty($route))
			{
				$wysiwyg = $route->aTitle(null,array('target'=>false));
				$data = array('html'=>$wysiwyg);
				$namePrimary = $this->namePrimary();
				$return = Html::div($namePrimary,array('insert','data'=>$data));
			}
		}
	...
	public static function makeSlug(Core\Col $col,array $row,?Core\Cell $cell=null,?array $option=null):string
	{
		...
		$slug = array('totalLength'=>$col->length());
