...
abstract class Form extends Core\RowAlias implements Main\Contract\Meta
{
	...
	public static $config = array(
		...
		'cols'=>array(
			'multiple'=>array('class'=>Core\Col\Yes::class),
			'json_fr'=>array('class'=>Site\Col\JsonForm::class,'exists'=>false),
			'json_en'=>array('class'=>Site\Col\JsonForm::class,'exists'=>false)),
		'@cms'=>array(
			'route'=>array(
	...
	public static function specificOperation(self $row):string 
	{
		...
		if($row->table()->hasPermission('specificOperation'))
		{
			...
			{
				$segment = array('table'=>$table,'order'=>'id','direction'=>'desc','filter'=>array('form_id'=>$row));
