...
class TinyMce extends Core\Col\TextareaAlias
{
	...
	public static $config = array(
		...
		'language'=>array('fr'=>'fr_FR'), // tableau pour convertir un code de language quid vers tinymce
		'tinymce'=>array( // config pour tinymce qui sera mis en attribut dans la tag textarea
			...
			'content_css'=>array("[public]/css/tinymce.css"),
			'convert_urls'=>false,
			'entity_encoding'=>'raw',
			...
			'style_formats'=>array(
				10=>array('title'=>'paragraph','format'=>'p','wrapper'=>true),
				11=>array('title'=>'superscript','format'=>'superscript'),
				13=>array('title'=>'header2','format'=>'h2'),
				14=>array('title'=>'header3','format'=>'h3'),
				15=>array('title'=>'header4','format'=>'h4'),
				16=>array('title'=>'header5','format'=>'h5')
			))
	);
	...
	public function tinymceData():array 
	{
		...
		if(is_array($return['style_formats']))
		{
			foreach ($return['style_formats'] as $key => $value) 
			{
				...
				$return['style_formats'][$key]['title'] = $lang->def(array('tinymce',$value['title']));
	...
	public function formComplex($value=true,?array $attr=null,?array $option=null):string 
	{
		...
		$attr = Base\Attr::append(array('tinymce','data'=>array('tinymce'=>$this->tinymceData())),$attr);
